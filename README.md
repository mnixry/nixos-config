...
> Every time I touch Ubuntu, I feel dirty. I'm typing commands thinking, *"Where is the declarative config? Why is this system mutable?"* It feels like Jenga—one wrong move and it's over. I was helping a colleague set up a legacy Python env today and I swear I got PTSD. I’m sweating, thinking about global namespace pollution. Then I watch him type `sudo pip install`... and he actually adds `--break-system-packages`. I watched the terminal output scrolling by and it just looked like imperative sin. Pure entropy. I asked him, *"Where is the Flake? How do we ensure reproducibility? How do we rollback?"* He just looked at me and said, **"Dude, chill. It's Debian. I'm just trying to run a script."**

Personal NixOS + Home Manager configuration for `mix-laptop-21tl`, managed via flakes.

This repo is intentionally opinionated and fairly hardware-specific (Intel Meteor Lake + NVIDIA PRIME + DisplayLink). It also uses an **ephemeral root** (`/` on tmpfs) and explicitly persists state under `/persistent` via [`preservation`](https://github.com/nix-community/preservation) (“erase your darlings” model).

## Highlights

- **Flakes + pinned inputs**: `nixos-unstable` + a `nixos-25.11` stable input (used for a few packages).
- **Nix implementation**: uses **Lix** (`config.nix.package = pkgs.lixPackageSets.stable.lix`), channels disabled.
...
- **Target host**: `mix-laptop-21tl` (from `vars/default.nix`).
- **Platform**: `x86_64-linux`.
- **Root model**: tmpfs `/` + persisted `/persistent`.
- **User management**: `users.mutableUsers = false` (users/passwords are configured declaratively; see `vars.user.initialHashedPassword`).
...
$EDITOR vars/default.nix   # hostname, user, boot/root devices, luks name, password hash
...
1. Partition for EFI + LUKS, then create a Btrfs filesystem inside LUKS.
2. Create Btrfs subvolumes: `@nix`, `@tmp`, `@swap`, `@persistent`.
3. Create the swapfile at `/swap/swapfile` (Btrfs swapfile rules apply) and mount subvolumes per `system/partition.nix`.
4. Copy this repo to the target filesystem and update `vars/default.nix`.
5. Install:
...
- `intel-xe`: force `xe` via kernel params for a specific device ID (see `system/hardware.nix`)
- `battery-saver`: enable powertop auto-tuning
...
# Optional: nh helpers (installed via Home Manager)
nh os switch .
nh clean all
...
- `/` is tmpfs; only paths listed in `preservation.nix` survive reboots under `/persistent`.
- When adding a new persisted path, move existing data from `/` into `/persistent` first (otherwise you can accidentally “hide” data behind the bind mount).
